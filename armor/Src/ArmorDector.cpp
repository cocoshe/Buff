	...
	*/
	...
	std::vector<LightDescriptor> lightInfos;
	{
		...
		*/
		...
		cv::threshold(_grayImg, binBrightImg, /*_param.brightness_threshold*/120, 255, cv::THRESH_BINARY);
		...
		*/
		...
		for (const auto &contour : lightContours)
		{
			...
			if (contour.size() <= 20 || lightContourArea < _param.light_min_area)
				continue;
			...
			for (int i = 0; i < 4; i++)
			{
				...
				/*查看计算的点的原始坐标*/
				//cout << "lightVertex[" << (i + 1) << "]" << lightVertex[i] << endl;
				/**/
			}
		...
		for (size_t i = 0; i < lightInfos.size(); i++)
		{
			for (size_t j = i + 1; (j < lightInfos.size()); j++)
			{
				...
				*/
				...
				float mean = (leftLight.angle + rightLight.angle)/2;
...
ArmorDescriptor::ArmorDescriptor(const LightDescriptor &lLight, const LightDescriptor &rLight, const int armorType, const cv::Mat &grayImg, float rotaScore, ArmorParam _param)
{
	...
	cout<<"sizeScore="<<normalized_area<<endl;
