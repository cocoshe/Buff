   ...
   // if (sParam.debug)
    ...
    if (classiMode==mySonNum) {
        for (size_t i = 0; i < armorContours_size; ++i)// 遍历每个轮廓
        {
            ...
            if (contour_area < 1000) continue; // 如果面积太小了，那就直接不管了
            ...
            if (findCount[i] == 1) {
                ...
                if ((son != -1) && (armorContours[son].size() > 2))//子轮廓ID不为-1，并且该轮廓点集数目必须大于2(才可以获得外接矩形)
                {
                    ...
                    {
                        line(src, p[j], p[(j + 1) % 4], Scalar(0, 255, 0), 2, 8);  //绘制最小外接矩形每条边
                    }
            ...
            } else if (findCount[i] > 2)//子轮廓>2，有可能是六边形
            {
                ...
                for (int j = 0; j < Sons_whratio.size(); j++) {
                            ...
                            //输出装甲板的Rectangle.angle
                            cout << "--------------SonRect angle---------------" << endl;
                            cout << "--------------SonRect angle : " << SonsRect[j].angle << endl;
                            cout << "--------------SonRect angle---------------" << endl;
    ...
    data .radius =radius;
    if (sParam.debug) {
        circle(src, data.armorCenter, 1, Scalar(255, 255, 255), 2);        //中心识别√
    //     if(data.preArmorCenter.x != 0 && data.preArmorCenter.y !=0)
    //     {
    //         circle(src , data.preArmorCenter , 35, Scalar(255,0,0),2);       //上一时刻中心√
    //     }
    //    // circle(src, data.preArmorCenter, 5, Scalar(255, 0, 0), 2);
    //    if(data.predictCenter .x != 0 && data.predictCenter.y != 0)
    //    {
    //        circle(src, data.predictCenter, 55, Scalar(255, 255, 0), 2);     //预测中心 
    //    }
    //     if(radius>0)
    //     {
