...
#define IMAGE_CENTER_X 480
#define IMAGE_CENTER_Y 400
...
//int GetBytesInCOM() const ;
    ...
    static bool sendTarget(Serial &serial, float x, float y, double distance, int armorMode, int isFind);
    bool sendBoxPosition( cv::Point2f aimPoint,Serial &serial, int findEnemy, double distance, int armorMode, cv::Point offset, char state);
    int getFd()
    {
        return fd;
    }
