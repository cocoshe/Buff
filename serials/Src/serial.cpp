 ...
 * @return 
*/
bool Serial::sendTarget(Serial &serial, float x, float y, double distance, int armorMode, int isFind)
{
    ...
    if(isFind == 1)
    {
        ...
        if(armorMode == BIG_ARMOR)
        {
            buff[16] =  '2';
        }
        else if(armorMode == SMALL_ARMOR)
        {
            buff[16] =  '1';
        }
 ...
 * @return
*/
bool Serial::sendBoxPosition(cv::Point2f aimPoint, Serial &serial, int findEnemy, double distance, int armorMode, cv::Point offset, char state)
{

/*
    double centerX = 480;   //MAT_WIDTH/2;
    double centerY = 400;   //MAT_HEIGHT/2;
*/

    double centerX = 320;   //MAT_WIDTH/2;
    double centerY = 240;   //MAT_HEIGHT/2;
...
*/
    ...
    float yaw;
    if(state == '4' || state == '5') yaw = atan(dx / FOCUS_PIXAL) * 180 / PI * 0.05;
    else yaw = atan(dx / FOCUS_PIXAL) * 180 / PI * 0.2;
    float pitch = atan(dy / FOCUS_PIXAL) * 180 / PI * 0.2;
    std::cout << "yaw:" << yaw << " pitch:" << pitch << std::endl << std::endl;
    // std::cout << "state:" << (count>0?1:(count==0?0:-1)) << std::endl;

    return sendTarget(serial, yaw, pitch, distance, armorMode, findEnemy);//yaw+
}
